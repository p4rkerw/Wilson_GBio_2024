# use neighborhood identified in step1 and degs from step2 to identify neighborhood-specific gsea
...
file <- here("ckd","spatial","markers","deg.celltype.xlsx")
...
sub <- deg %>%
  dplyr::filter(celltype == "PT_INJ", p_val < 0.05)
...
require(DOSE)
pdf(here("ckd","spatial","plots","gsea1.pdf"))
dotplot(gse, showCategory=10, split=".sign") + facet_grid(.~.sign)
dev.off()
  
saveRDS(gse, here("ckd","spatial","step3_gsea.rds"))  
...
pdf(here("ckd","spatial","plots","gsea2.pdf"))
