...
file <- here("ckd","rna_aggr_prep","kpmp","markers","deg_loy_vs_xx.age_adjust.xlsx")
...
sub <- deg %>%
  dplyr::filter(celltype == "PT", p_val < 0.05)
...
require(DOSE)
pdf(here("ckd","rna_aggr_prep","kpmp","plots","gsea_loy_vs_xy.PT.age_adjust.pdf"))
dotplot(gse, showCategory=10, split=".sign") + facet_grid(.~.sign)
dev.off()
  
saveRDS(gse, here("ckd","rna_aggr_prep","step2_gsea_loy_vs_xy.PT.age_adjust.rds"))  
...
pdf(here("ckd","rna_aggr_prep","kpmp","plots","gsea_loy_vs_xx2.pdf"))
...
require(DOSE) 
saveRDS(gse, here("ckd","rna_aggr_prep","kpmp",paste0("step2_",celltype_sel,"_gsea.rds")))  
...
pdf(here("ckd","rna_aggr_prep","kpmp","plots",paste0(celltype_sel,"_gsea.pdf")))
plot <- dotplot(gse, showCategory=terms$Description, split=".sign") + 
  facet_grid(.~.sign) +
  theme_bw() +
  theme(legend.pos = "none", plot.title = element_text(face="bold"), axis.text = element_text(color="black")) +
  ggtitle(celltype_sel)
print(plot)  
dev.off()  
...
############################
file <- here("ckd","rna_aggr_prep","kpmp","markers","deg_loy_vs_xx.xlsx")
sheets <- getSheetNames(file)
deg <- lapply(sheets, function(sheet) {
  tryCatch({df <- read.xlsx(file, sheet)
            df$celltype <- sheet
...
require(DOSE)
pdf(here("ckd","rna_aggr_prep","kpmp","plots","gsea_loy_vs_xx.pdf"))
dotplot(gse, showCategory=10, split=".sign") + facet_grid(.~.sign)
dev.off()
  
saveRDS(gse, here("ckd","rna_aggr_prep","step2_gsea.rds"))  
